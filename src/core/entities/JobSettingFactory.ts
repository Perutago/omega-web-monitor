...
export default class JobSettingFactory {
    ...
    static fromJson(json: any) {
        if (json.type === XpathJobSetting.type) {
            return new XpathJobSetting(json.id, json.name, json.cronTime, json.url, json.enabled, json.xpath);
        }
        else if (json.type === RegexJobSetting.type) {
            return new RegexJobSetting(json.id, json.name, json.cronTime, json.url, json.enabled, json.regex);
        }
        else {
